...
study_group_tf_idf <- study_long %>%
  count(구분, word, sort = TRUE) %>% # 문서 -> 단어 순으로
  bind_tf_idf(word, 구분, n)         # 단어 -> 문서 순으로
...
study_group_tf_idf <- study_group_tf_idf %>% 
  bind_log_odds(set = 구분, 
                feature = word, 
                n = n) %>%
...
study_group_count <- 
  ...
  arrange(desc(tf_idf)) %>%  #tf_idf로 정렬
  ungroup()
...
# study_group_count %>%
#   write_excel_csv("study_group_count")
